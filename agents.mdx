---
title: "Squad"
description: "What a squad member is, how to configure one, and how they work in Dash."
---
...
You configure each squad member in Mission Control's **Deploy** wizard (and edit it later from the squad member's **Configuration** tab). Behind the scenes, a squad member's saved configuration looks like this:
...
Mission Control saves your squad members to `~/.dash/gateway/agents.json` and the gateway restores them on restart.
...
You can **start**, **stop**, **restart**, and **remove** squad members at any time from Mission Control's **Squad** page — click a squad member to manage it. For automation, the same operations are available on the [Management API](/api-reference#agents).
...
You can run as many squad members as you need. Each one is independent — different model, different instructions, different tools. Deploy each from Mission Control; their configurations look like this:
...
<CardGroup cols={2}>
  ...
  <Card title="Configuration" icon="gear" href="/configuration">
    Squad member settings, the data directory layout, and standalone gateway flags.
  </Card>
</CardGroup>
