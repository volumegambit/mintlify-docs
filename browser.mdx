...
| `browser_screenshot` | Capture the page or one element as an image. Prefer browser_snapshot; use this when layout or visuals matter. | `ref` (optional) — Capture only this element<br />`fullPage` (optional) — Capture the whole scrollable page instead of the viewport, up to 8000 px tall. |
...
When an agent calls `browser_screenshot`, the tool row in chat shows a thumbnail of the capture on every client, including iPhone, iPad, and Android. In Desktop and the web client, click it to enlarge it; click again to shrink it. On iPhone, iPad, and Android, tap it to open the screenshot full screen.

A `fullPage` screenshot can be taller than one screen — up to the tool's 8000 px cap. On iPhone, iPad, and Android, a tall screenshot opens filled to the screen's width instead of shrunk down to fit, so it's readable; pinch to zoom in further, and double-tap to toggle between that width-fill view and the whole page. Close it with the **×** button; on Android, tapping the image also closes it, as long as you haven't pinched in past where it opened. On iPhone and iPad, **Share** and **Save to Photos** save the full screenshot, not just the thumbnail. The thumbnail is kept with the conversation.
