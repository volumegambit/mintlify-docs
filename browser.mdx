...
<Note>
  The browser doesn't try to pass as a person. It leaves the browser's automation signals in place and makes no attempt to get around bot checks. When a site asks for a CAPTCHA or a 2FA code, a person solves it through the live view — see [Hand off a CAPTCHA or 2FA prompt](#hand-off-a-captcha-or-2fa-prompt).
</Note>
...
If you install Chrome or Edge later, your agents switch to it and the managed browser stops updating. It stays on disk; the **Browser** settings page tells you so.
...
| Browsers open at once, per HQ | **3** by default, across every agent and conversation. Opening one more fails with `this HQ is already running 3 browser sessions`. Change it with `BROWSER_MAX_SESSIONS`. |
...
- **Profiles hold real sign-ins.** A profile's cookies stay signed in to the sites the agent used. The profile folders under `~/.dash/gateway/browser-profiles/` are readable only by the user account the HQ runs as, but anyone signed in as that user can read them. Delete profiles you no longer need.
...
| `BROWSER_EXECUTABLE_PATH` | Full path to the browser to use. It's the first place the HQ looks. If nothing exists at that path, the HQ falls back to finding Chrome or Edge as usual. |
