...
In a chat with an agent that has opened a browser, open **View browser** — in Desktop it's a globe icon in the chat's status bar; in the web client it's a button in the top bar; on iPhone, iPad, and Android it's in the conversation's menu. In Desktop the view opens as a panel beside the chat; in the web client it's a column beside the chat, or a full-screen overlay when the window is 768 px wide or narrower; on iPhone, iPad, and Android it opens full screen.
...
On iPhone, iPad, and Android: tap to click, and drag to scroll. A text field above the keyboard sends what you type, and its Return key sends the page's Enter. There's no paste and no hover on mobile. If the page opens a dialog, the view shows the same **The page is showing a dialog: "…"** message.
