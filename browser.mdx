...
A `fullPage` screenshot can be taller than one screen — up to the tool's 8000 px cap. On iPhone, iPad, and Android, a tall screenshot opens filled to the screen's width instead of shrunk down to fit, so it's readable; pinch to zoom in further, and double-tap to toggle between that width-fill view and the whole page. Close it with the **×** button; on Android, tapping the image also closes it, while the whole screenshot is showing. On iPhone and iPad, **Share** and **Save to Photos** save the whole screenshot, not just the thumbnail. The thumbnail is kept with the conversation.
