...
Keys you add there go into the same encrypted store on your HQ, so they show up in Desktop too, and the other way round. Once saved, a key is never shown again anywhere: the apps list only each key's name (the iPhone, iPad, and Android apps also show when it was added).
...
Your HQ does the sign-in and keeps the account's tokens; the app never sees them. Connected accounts are listed as **Signed in with Anthropic** or **Signed in with OpenAI**, under the account's email where your HQ knows it (and your ChatGPT plan), and stay signed in until you remove them. A Claude account connected before this was added shows its email once you sign in again.
