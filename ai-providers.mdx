...
Keys you add there go into the same encrypted store on your HQ, so they show up in Desktop too, and the other way round. Once saved, a key is never shown again anywhere: the apps list only each key's name. Signing in with a Claude or Codex login still happens in Desktop, because it opens a browser on the HQ's computer. Those keys appear in the apps marked **Signed in on Desktop**, and you can remove or replace them there.
