---
title: "Architecture"
description: "How DashSquad is structured, how data flows, and how agents process messages."
---

DashSquad is made up of two main pieces that work together to keep your agent team running.
...
graph TB
  ...
  subgraph infra ["Your Machine"]
    gateway["HQ<br/><small>Hosts agents in-process<br/>Management :9300 · Channels :9200</small>"]
...
DashSquad has two main components:
...
DashSquad is flexible about where each piece runs.

**Single machine** — run everything locally. Launch Desktop, which spawns the HQ as a child process. Good for development and personal use.

**Headless** — run the HQ standalone on a VPS, server, or in Docker: `npm run gateway`. Manage it through its [Management API](/api-reference), or point Desktop at the management URL for remote management.
...
<AccordionGroup>
  ...
  <Accordion title="Management API" icon="tower-control">
    HTTP server (port 9300) for operational control. Provides health checks (`/health`) plus CRUD for agents, channels, credentials, and models. Used by Desktop to deploy and manage agents. See the [Management API reference](/api-reference).
  </Accordion>
  <Accordion title="HQ" icon="server">
    The main entry point. A single process that hosts agents in memory, runs both the channel server and management API, and owns all messaging platform connections. Creates agents with their assigned models and tools, and routes messages from external platforms (Telegram, WhatsApp) to agents. Agents, channels, and credentials are created at runtime through the management API rather than a config file. Supports CLI flags (`--token`, `--data-dir`, …) for standalone use — see [Configuration](/configuration#running-the-hq-standalone).
  </Accordion>
  <Accordion title="Desktop" icon="grid-2">
...
DashSquad for iOS, capability-aware Desktop, and the web client share the
HQ-authoritative conversation history. Android shares canonical conversations with Mobile API v2,
including the active response, replay, and the accepted input queue; on older HQs it keeps its
legacy private chat sessions. The [sub-agent](#sub-agents) interface — rows, transcripts, and the tasks list — is
on iOS, the web client, and Desktop only.

For HQs that advertise `conversation-sync-v1`, the HQ owns conversation metadata,
...
Mobile API v2 keeps one active run per conversation and a durable input queue at the HQ. A **Steer**
is delivered at a safe boundary inside that run and starts a new transcript segment. A **Follow Up**
waits for a later run. Clients display committed state; they do not advance the queue themselves.
Accepted inputs and their message identities survive reconnection and process restart.

Stopping the active v2 run advances the queue. Failure pauses remaining Follow Ups until an explicit
**Resume Follow Ups**. The older v1 pending-work API remains compatible, including its distinct
Stop-pauses and priority-interruption behavior. Both APIs use the same queue and execution owner.

V2 subscriptions replay a conversation-wide `v2Seq`. V1 retains its own `seq`; the two cursors are
independent. A snapshot includes a replay watermark so a client can combine saved messages and new
events without duplicating text. See [the v2 API](/api-hq-mobile#steer-and-follow-up-mobile-api-v2).

A capable client can also let the HQ choose automatically instead of asking every time. It sends the
message with a request to route it, rather than a fixed Steer or Follow Up. At the HQ, a decision
model (TypeSafe's Jev, called through OpenRouter) reads the run's current task and the response so
far, and answers with a choice and a confidence. A confident answer is applied immediately through
the same Steer/Follow Up path and labeled as automatic. Anything else — low confidence, a timeout,
or no decision model configured — is returned to the client as a choice, with the model's best guess
attached, so the person decides.

## HQ APIs

The HQ runs two loopback servers for host administration and a separate pinned-TLS listener
for native devices:
...
| Mobile LAN server | HTTPS / WSS | 9400 | Phone-scoped bearer | `/mobile/v1`, `/mobile/v2`, and `/ws/chat` only |
...
the encrypted `Authorization` header for both `/mobile/v1` and `/mobile/v2` HTTP/SSE requests and `/ws/chat`
streaming. Current clients probe `GET /mobile/v2/health` for `chat-input-queue-v1` and verify the
HQ identity. They use v1 only when v2 is explicitly unavailable, not after authentication or TLS
failures.

## Remote access identity

When you use the [hosted relay](/remote-access) to reach your agents from your phone,
the HQ authenticates itself with its **own cryptographic identity** rather than a
shared password. On first start, the HQ generates an Ed25519 key pair and stores
the private half on its own disk (owner-readable only); the private key never leaves the
machine. Only the matching **public** key is ever registered with DashSquad, so the hosted
service holds nothing that could impersonate your HQ.

HQs are owned by an **organization**, not by an individual login. When you set up
remote access you sign in to DashSquad and act on behalf of an organization; the HQs and
...
Each HQ also has a **permanent address** on the relay — the hostname you chose when
you set up remote access. That address is globally unique and is claimed for good: it's
never reassigned and never recycled, even if you later remove the HQ, so a paired
phone always resolves the same HQ and a name can't be taken over by someone else.

Each time the HQ dials out to the relay, it presents its relay address token
together with a short-lived proof signed by that private key. The relay checks, offline,
that the token was issued by DashSquad for that exact address **and** that the dialer actually
...
HQ renews its own token in the background — on a timer before it expires, on the next
dial after a rejected one, and on boot after being offline. So an HQ that's been
powered off for days or weeks reconnects on its own, with no re-enrollment and without
Desktop needing to be open.
...
<Steps>
  <Step title="Discover and load">
    ...
    `chat-resume-v1`, then creates or loads the HQ's canonical conversation.
  </Step>
...
Desktop and the DashSquad phone app see the same active turn when they use the same capable
HQ. Either client can watch replayed output, answer a pending question, or explicitly cancel
...
The canonical conversation record is what Desktop and paired phone apps render. It owns
...
history shared by Desktop and mobile clients.

Cross-device conversation state is canonical in the HQ's SQLite database:
...
tombstones, the active-turn lease, and the ordered replay journal. DashSquad mobile and capability-aware
Desktop clients cache this state and reconcile from the Management API rather than treating
local chat history as authoritative. When the HQ does not advertise the required capabilities,
Desktop falls back to legacy connection-owned chat.
...
The HQ assigns a monotonically increasing sequence across all turns in a conversation. A
