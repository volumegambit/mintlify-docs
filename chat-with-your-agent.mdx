...
On a current HQ, the message box stays editable while your agent works. Type your message and press
**Send**.

On Desktop and the web client, DashSquad decides how to deliver it:

- If your message changes or narrows the work in progress, it becomes a **Steer**: the agent picks it
  up at its next safe boundary. It is labeled **Auto-steered**.
- If it asks for separate work, it becomes a **Follow Up** in the shared queue, labeled **Auto-queued**.
- If DashSquad is not sure, it asks. The choice opens with its best guess marked **Suggested**, and your
  message stays in the box until you pick one.

To choose yourself instead, press **Cmd+Return** (Ctrl+Enter on Windows and in the web client)
instead of **Send**. On iPhone, iPad, and Android, **Send** always opens the choice today —
automatic routing is coming to those apps in a later release. The choice offers:
...
To make the call, the HQ sends your new message's text — along with a short excerpt of the current
task and the reply in progress — to a decision model (TypeSafe's Jev, reached through OpenRouter
using your OpenRouter key). Images are never sent. Automatic routing needs that OpenRouter key on
your HQ; without one, or with **Settings → Chat → While a response is running → Always ask** in
Desktop, every message asks instead — switch to **Always ask** any time you'd rather turn this off.
