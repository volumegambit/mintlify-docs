...
```mermaid
graph LR
    P["Messaging Platform<br/><small>Telegram, WhatsApp</small>"] -->|Bot API| GW["HQ"]
    GW --- A["Squad"]
    A -->|response| GW
    GW -->|Bot API| P
```
...
<Steps>
  ...
  <Step title="Add the bot in Desktop">
...
<Accordion title="Without Desktop">
  Channels aren't configured with a file — they're created at runtime through the HQ's [Management API](/api-reference#channels). Desktop is just a friendly front-end for that API.

  To set one up against a standalone HQ, `POST /channels` with the adapter, token, routing rules, and access lists, then enable it. See the [Management API](/api-reference#channels) reference.
</Accordion>
...
<Steps>
  ...
  <Step title="Add the connection in Desktop">
...
<AccordionGroup>
  ...
  <Accordion title="Running the HQ manually">
    ```bash
    npm run gateway
    ```

    The HQ starts on its data directory (`~/.dash/gateway`), reconnects any saved channels, and begins routing. Create or edit channels through its [Management API](/api-reference#channels), and supply bot tokens the same way (`POST /credentials`). It runs until stopped. See [Configuration](/configuration#running-the-hq-standalone) for the available flags.
  </Accordion>
