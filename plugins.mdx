...
DashSquad plugins follow the **Claude Code plugin layout**, so a plugin published for Claude Code works in DashSquad unchanged.
...
Installing a plugin brings someone else's instructions — and potentially their code — onto your HQ. DashSquad treats every install as **untrusted by default** and layers several defenses:
...
A **marketplace** is a published catalog of plugins you can browse and install by name. You point DashSquad at a marketplace source — a local path, a URL, or a git repo — and DashSquad reads its `marketplace.json` to list the plugins it offers.
