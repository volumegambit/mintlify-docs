...
| `MCP_OAUTH_REDIRECT_URI` | Where connector sign-in pages send you back. Defaults to `dash://mcp-oauth`, which Desktop and the mobile apps catch. Change it only if a service refuses app redirects and you host a page that forwards to `dash://mcp-oauth` with the query unchanged. |
