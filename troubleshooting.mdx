...
<AccordionGroup>
  <Accordion title="No browser available">
    ...
    **Fix:** The HQ found neither Chrome nor Edge on its machine, and no managed browser is installed. Install Google Chrome or Microsoft Edge, or open **Settings → Browser** in Desktop and click **Download browser** (about 190 MB). Then click **Check again**. If your browser is somewhere unusual, point `BROWSER_EXECUTABLE_PATH` at it. See [Browser → Get a browser](/browser#get-a-browser).
  </Accordion>
  ...
  <Accordion title="The HQ is already running 3 browser sessions">
    **Error:**
    ```
    this HQ is already running 3 browser sessions
    ```

    **Fix:** The HQ caps how many browsers are open at once, across every agent. Ask an agent to close a browser it no longer needs, wait for an idle one to close, or raise the cap with `BROWSER_MAX_SESSIONS` (up to `10`). See [Browser → Limits](/browser#limits).
  </Accordion>
