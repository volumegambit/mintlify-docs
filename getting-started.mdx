...
description: "Install DashSquad, launch Desktop, and deploy your first AI agent."
---
...
- **Node.js 22+** — DashSquad uses ESM and modern language features requiring Node 22 or later
- **An LLM API key** — for example, sign up at [console.anthropic.com](https://console.anthropic.com) for an Anthropic key

## Getting Your Anthropic API Key

DashSquad works with Claude from Anthropic out of the box. To use it, you need an Anthropic account and an API key.
...
**Step 3:** Click **Create Key**, give it a name (e.g. "DashSquad"), and copy the key — it starts with `sk-ant-`.

<Note>
  **Keep your API key private.** Never share it or commit it to version control. DashSquad stores it encrypted on your machine.
</Note>
...
Your HQ keeps the canonical conversation history. When Desktop and DashSquad for iOS are
...
In Desktop, open **Settings → Devices**. The **Pair Device** card links the DashSquad Android
...
DashSquad for iOS connects differently: enable [Remote Access](/remote-access), then open the app and
sign in with your DashSquad account to choose your HQ. The native iPhone and iPad client supports
shared, resumable history and agent administration — see [DashSquad for iPhone and iPad](/ios) for
